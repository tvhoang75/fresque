...
CLOUD="cloud0 | cloud0cache | cloud1 | cloud1cache | cloud2 | cloud3 | cloud_pined2 | iot_cloud1 | iot_cloud2 | "
DISPATCHER="collector | collector_pined2 | dispatcher | dispatcher0 | dispatcher1 | dispatcher2 | dispatcher3 | iot_collector | keygen |"
GENEGATOR="generator | generator2 | generator3 | "
MERGER="merger | merger3 | "
CHECKER="checker | checker3 | "
SERVERNODE="servernode1 | servernode2 | servernode3 | "
CONSUMER="consumer | consumercloud0 | consumercloud0cache | consumercloud1cache | iot_consumer | " 
...
then
       echo "#Start building ..."
       mvn -f $CLASSPATH_SOURCE/pom.xml clean compile assembly:single 
#        mvn -f $CLASSPATH_SOURCE/pom.xml  
	   echo "#Copying BigLog-1-jar-with-dependencies.jar to current folder"
       cp $CLASSPATH_SOURCE/target/BigLog-1-jar-with-dependencies.jar ./$APP_NAME
	exit;
fi

if [ $1 = "keygen" ]
then
		echo "#Start key pair generation..."
        java $FLAG -classpath $APP_NAME main.crypto.jpaillier.KeyPairBuilder
        exit;
fi

if [ $1 = "iot_cloud1" ]
then
	echo "#Start iot cloud 1..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_IOT_CLOUD1.Cloud1
        exit;
fi

if [ $1 = "iot_cloud2" ]
then
		echo "#Start iot cloud 2..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_IOT_CLOUD2.Cloud2
        exit;
fi

if [ $1 = "iot_collector" ]
then
	echo "#Start iot collector ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_IOT_COLLECTOR.Collector
        exit;
fi

if [ $1 = "iot_consumer" ]
then
	echo "#Start iot consumer ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_IOT_CONSUMER.Consumer
        exit;
fi

if [ $1 = "cloud2_iot" ]
then
	echo "#Start cloud pined 2..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.pined2.Cloud
        exit;
fi
...
if [ $1 = "collector_pined2" ]
then
	echo "#Start collector pined 2..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_COLLECTOR.pined2.Collector
        exit;
fi
...
then
	echo "#Start cloud 2 ..."
        java -classpath $APP_NAME $CLASSPATH_CLOUD_2.Cloud
        exit;
fi
...
then
        echo "#Start merger ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_MERGER.Merger
        exit;
fi
...
then
        echo "#Start checker ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CHECKER.Checker
        exit;
fi
...
then
        ...
        java $FLAG -classpath $APP_NAME $CLASSPATH_DISPATCHER.Dispatcher
        exit;
fi

if [ $1 = "servernode2" ]
then
        echo "#Start  server node 2..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_SERVERNODE_2.ServerNode
        exit;
fi

if [ $1 = "generator2" ]
then
        echo "#Start data generator 2 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_DATASOURCE_2.DataGenerator
        exit;
fi

if [ $1 = "worker2" ]
then
        echo "#Start data worker 2 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD_2.worker.Worker
        exit;
fi

if [ $1 = "storagemanager2" ]
then
        echo "#Start data storage manager 2 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD_2.storage.StorageManager
        exit;
fi

if [ $1 = "generator" ]
then
        echo "#Start data generator pined ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_DATASOURCE.pined.DataGenerator
        exit;
fi
...
then
	...
	java $FLAG -classpath $APP_NAME $CLASSPATH_DATASOURCE.distributing1.DataGenerator
	exit;
fi
...
then
	echo "#Start data generator ..."
	java $FLAG -classpath $APP_NAME $CLASSPATH_DATASOURCE.distributing0.DataGenerator
	exit;
fi
...
then
	echo "#Start cloud 0 ..."
	java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.indextemplate.distributing0.Cloud
	exit;
fi

if [ $1 = "cloud0cache" ]
then
	echo "#Start cloud 0 with cache..."
	java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.indextemplate.caching0.Cloud
	exit;
fi

if [ $1 = "cloud1cache" ]
then
	echo "#Start cloud 1 with cache..."
	java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.indextemplate.caching1.Cloud
	exit;
fi

if [ $1 = "cloud" ]
then
        echo "#Start cloud pined ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.pined.Cloud
        exit;
fi
...
then
	echo "#Start cloud 1 ..."
	java $FLAG -classpath $APP_NAME $CLASSPATH_CLOUD.indextemplate.distributing1.Cloud
	exit;
fi
if [ $1 = "collector" ]
then
        echo "#Start collector pined ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_COLLECTOR.pined.Collector
        exit;
fi
if [ $1 = "dispatcher0" ]
then
        echo "#Start CollectorIndexTemplate ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_COLLECTOR.indextemplate.distributing0.CollectorIndexTemplate
        exit;
fi
if [ $1 = "dispatcher1" ]
then
        echo "#Start dispatcher 1 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_COLLECTOR.indextemplate.distributing1.Dispatcher
        exit;
fi
if [ $1 = "servernode1" ]
then
        echo "#Start ServerNode 1 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_COLLECTOR.indextemplate.distributing1.ServerNode
        exit;
fi
if [ $1 = "consumer" ]
then
        echo "#Start Consumer ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CONSUMER.pined.Consumer
        exit;
fi
if [ $1 = "consumercloud0" ]
then
        echo "#Start consumer cloud 0 ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CONSUMER.indextemplate.distributing0.CloudConsumer
        exit;
fi

if [ $1 = "consumercloud0cache" ]
then
        echo "#Start consumer cloud 0 with cache ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CONSUMER.indextemplate.caching0.CloudConsumer
        exit;
fi

if [ $1 = "consumercloud1cache" ]
then
        echo "#Start consumer cloud 1 with cache ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CONSUMER.indextemplate.caching1.CloudConsumer
        exit;
fi


if [ $1 = "ConsumerIndexTemplate" ]
then
        echo "#Start ConsumerIndexTemplate ..."
        java $FLAG -classpath $APP_NAME $CLASSPATH_CONSUMER.indextemplate.ConsumerIndexTemplate
        exit;
fi
